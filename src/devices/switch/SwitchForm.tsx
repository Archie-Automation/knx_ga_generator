...
import { SwitchDevice, SwitchOutput, DeviceCategory, AnyDevice, DimmerDevice, BlindDevice } from '../../types/common';
...
interface Props {
  onSave: (device: SwitchDevice) => void;
  onUpdate?: (device: SwitchDevice) => void;
  count: number;
  onCountChange: (count: number) => void;
  existing: SwitchDevice[];
}
...
export const SwitchForm = ({ onSave, onUpdate, count, onCountChange, existing }: Props) => {
  ...
  // Get available switch groups from template (teachByExampleConfig)
  const switchGroups = useMemo(() => {
    if (!template?.teachByExampleConfig?.categories?.switching) {
      // If no teachByExampleConfig, return empty array (no groups to select)
      return [];
    }
    const switchingConfig = template.teachByExampleConfig.categories.switching;
    if (Array.isArray(switchingConfig)) {
      // Multiple groups - return all with their names
      return switchingConfig.map((group, idx) => ({
        index: idx,
        name: group.groupName || `Schakelen ${idx + 1}`,
        config: group
      }));
    } else {
      // Single group - use groupName from teachByExampleConfig, with fallback
      // IMPORTANT: groupName should be taken from the config
      const groupName = switchingConfig.groupName || 'Schakelen';
      return [{
        index: 0,
        name: groupName,
  ...
  // Check if there's only one switch group (hide selector if true)
  const hasSingleSwitchGroup = switchGroups.length === 1;

  // Check if dimming is linked to switching
  const isDimmingLinkedToSwitching = useMemo(() => {
    if (!template?.teachByExampleConfig?.categories?.dimming) return false;
    ...
    return dimmingGroups.some(dg => dg.linkedToSwitching === true);
  }, [template]);

  // Get available dim groups from template when dimming is linked to switching
  const dimGroups = useMemo(() => {
    if (!isDimmingLinkedToSwitching || !template?.teachByExampleConfig?.categories?.dimming) {
      return [];
    }
    const dimmingConfig = template.teachByExampleConfig.categories.dimming;
    const dimmingGroups = Array.isArray(dimmingConfig) ? dimmingConfig : [dimmingConfig];
    // Only return dim groups that are linked to switching
    return dimmingGroups
      .map((group, idx) => ({ group, index: idx }))
      .filter(({ group }) => group.linkedToSwitching === true)
      .map(({ group, index }) => ({
        index: index,
        name: group.groupName || `Dimgroep ${index + 1}`
      }));
  }, [template, isDimmingLinkedToSwitching]);
  ...
  useEffect(() => {
    ...
    if (prevLangRef.current !== lang && prevLangRef.current !== undefined && prevLangRef.current !== null && onUpdate) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        const devicesToSave: Array<{ device: SwitchDevice; isDraft: boolean }> = [];
        ...
        setDraftActors(currentDraftActors => {
          existing.forEach(device => {
            ...
            if (draft) {
              ...
              const deviceToSave: SwitchDevice = {
                id: draft.id,
                category: 'switch',
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            const updatedDevices = allDevices.switch as SwitchDevice[];
            const drafts = new Map<string, ActorData>();
            updatedDevices.forEach(device => {
              drafts.set(device.id, {
                ...
                physicalAddress: device.physicalAddress,
                channelCount: device.channelCount,
                outputs: device.outputs.map((output: SwitchOutput) => {
  ...
  useEffect(() => {
    setActors(prev => {
      ...
      // This happens when an actor is saved - it gets added to existing and should be removed from new actors
      ...
      const actorsToKeep = prev.filter(a => {
        // Always keep actors without physical address (they're still being edited)
        if (!a.physicalAddress || !a.physicalAddress.trim()) {
          return true;
        }
        // Only remove if the physical address matches an existing device (actor was saved)
      ...
      if (desiredNewActorsCount > actorsToKeepCount) {
        ...
        const allAddresses: string[] = [
          ...(allDevices.switch as SwitchDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== ''),
          ...(allDevices.dimmer as DimmerDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== ''),
          ...(allDevices.blind as BlindDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== ''),
          ...actorsToKeep.map(a => a.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '')
        ];
        ...
        for (let i = actorsToKeepCount; i < desiredNewActorsCount; i++) {
          ...
          newActors.push({
            id: uid(),
            manufacturer: '',
            model: t('defaultSwitchModel') || 'Switch Actuator',
  ...
  // Auto-create outputs when channelCount changes for any actor
      ...
      if (actor.id === actorId) {
        ...
        for (let i = 0; i < actor.outputs.length && i < newCount; i++) {
          ...
          newOutputs.push({
            ...
            // Auto-set switchGroupIndex to 0 if there's only one group and it's not already set
            switchGroupIndex: hasSingleSwitchGroup && (output.switchGroupIndex === undefined || output.switchGroupIndex === null) ? 0 : output.switchGroupIndex,
            // Auto-set dimGroupIndex to 0 if dimming is linked to switching and it's not already set
            dimGroupIndex: isDimmingLinkedToSwitching && dimGroups.length > 0 && (output.dimGroupIndex === undefined || output.dimGroupIndex === null) ? 0 : (output.dimGroupIndex ?? undefined)
          });
        }
        ...
        for (let i = actor.outputs.length; i < newCount; i++) {
          ...
          newOutputs.push({
            ...
            remarks: '',
            isReserve: false,
            // Auto-set switchGroupIndex to 0 if there's only one group
            switchGroupIndex: hasSingleSwitchGroup ? 0 : undefined,
            // Auto-set dimGroupIndex to 0 if dimming is linked to switching and there are dim groups
            dimGroupIndex: isDimmingLinkedToSwitching && dimGroups.length > 0 ? 0 : undefined
          });
        }
  ...
  const updateActor = (actorId: string, updates: Partial<ActorData>) => {
    ...
    if (updates.physicalAddress && effectiveMfr !== 'theben') {
      ...
      // Check against other Switch devices (exclude current if updating)
      const otherSwitchAddresses: string[] = [
        ...(allDevices.switch as SwitchDevice[]).filter(d => d.id !== actorId).map(d => d.physicalAddress),
      ...
      const dimmerAddresses: string[] = (allDevices.dimmer as DimmerDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '');
      
      if (otherSwitchAddresses.includes(updates.physicalAddress) || 
  ...
  const updateOutput = (actorId: string, outputId: string, key: keyof SwitchOutput, value: string | boolean | number) => {
      ...
      if (actor.id === actorId) {
        ...
        const updatedOutputs = actor.outputs.map(o => {
          if (o.id === outputId) {
            ...
            // If channelName changed, check for duplicates
            if (key === 'channelName') {
        ...
        if (typeof roomNameToSync === 'string' && typeof roomAddressToSync === 'string' && roomNameToSync && roomAddressToSync) {
          const syncRoomAddress: string = (roomAddressToSync as string).trim();
          const syncRoomName = roomNameToSync;
          if (syncRoomAddress && syncRoomAddress.length > 0) {
            ...
            updatedOutputs.forEach((otherOutput, otherIdx) => {
              if (otherOutput.id !== outputId && 
                  ...
                  otherOutput.roomAddress === syncRoomAddress &&
                  'roomName' in otherOutput) {
                updatedOutputs[otherIdx] = { ...otherOutput, roomName: syncRoomName };
              }
            });
            
            // Sync to all other devices/actors with the same roomAddress
            syncRoomNameToAllDevices(syncRoomAddress, syncRoomName, actorId, outputId);
          }
        }
  ...
  const getSortedOutputs = (outputs: SwitchOutput[]): SwitchOutput[] => {
  ...
  const saveActor = (actor: ActorData, index: number) => {
    ...
    if (actor.manufacturer !== 'theben') {
      const { devices: allDevices } = useAppStore.getState();
      const otherSwitchAddresses: string[] = [
        ...(allDevices.switch as SwitchDevice[]).map(d => d.physicalAddress),
        ...actors.filter((_, idx) => idx !== index).map(a => a.physicalAddress)
      ].filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '');
      
      const dimmerAddresses: string[] = (allDevices.dimmer as DimmerDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '');
      
      if (otherSwitchAddresses.includes(actor.physicalAddress) || 
        ...
        if ('outputs' in device && device.outputs) {
          const updatedOutputs = device.outputs.map(output => {
            if ('roomAddress' in output && 
              ...
              if (excludeDeviceId && excludeOutputId && device.id === excludeDeviceId && output.id === excludeOutputId) {
                return output;
              }
              hasChanges = true;
  ...
  const findRoomNameByAddress = (roomAddress: string, currentDeviceId?: string, currentOutputId?: string): string | null => {
    ...
    // First check the cache (fastest lookup)
    ...
    // Check all actors (new actors that haven't been saved yet)
      ...
      if (draftActor.outputs) {
        for (const output of draftActor.outputs) {
          if ('roomAddress' in output && output.roomAddress === roomAddress && 'roomName' in output) {
            ...
            if (currentDeviceId && currentOutputId && draftDeviceId === currentDeviceId && output.id === currentOutputId) {
              continue;
            }
            const roomName = output.roomName;
            if (roomName && roomName.trim()) {
    ...
    console.log('[SwitchForm] No roomName found for roomAddress:', roomAddress);
    return null;
  };

  // Update draft output
  const updateDraftOutput = (deviceId: string, outputId: string, key: keyof SwitchOutput, value: string | boolean | number, autoFillRoomName: boolean = true) => {
    const currentDraft = draftActors.get(deviceId);
    if (!currentDraft) return;
    const reserveText = t('reserve') || 'reserve';
    ...
    const updatedOutputs = currentDraft.outputs.map(o => {
      if (o.id === outputId) {
        ...
        if (key === 'isReserve' && value === true) {
        ...
        } else if (key === 'roomAddress' && typeof value === 'string' && autoFillRoomName) {
          // When roomAddress is entered, check if there's an existing roomName for this address
          ...
          // Update the cache when roomName is changed
          if (updated.roomAddress && typeof updated.roomAddress === 'string' && updated.roomAddress.trim()) {
            const { updateRoomAddressCache } = useAppStore.getState();
            updateRoomAddressCache(updated.roomAddress, value);
          }
          // When roomName is changed, we'll sync it after updating all outputs
          if (typeof updated.roomAddress === 'string') {
            roomAddressToSync = updated.roomAddress;
          }
          roomNameToSync = value;
        }
    ...
    // If roomName was changed, sync it to all other outputs with the same roomAddress
    if (roomNameToSync && typeof roomNameToSync === 'string' && roomAddressToSync && typeof roomAddressToSync === 'string') {
      const syncRoomAddress: string = (roomAddressToSync as string).trim();
      if (syncRoomAddress && syncRoomAddress.length > 0) {
        const syncRoomName = roomNameToSync;
        if (syncRoomAddress.length > 0) {
          ...
          // Sync to all other devices/actors with the same roomAddress
          syncRoomNameToAllDevices(syncRoomAddress, syncRoomName, deviceId, outputId);
        }
      }
    }
  ...
  const saveDraftChanges = (deviceId: string) => {
    ...
    if (draft.physicalAddress && draft.manufacturer !== 'theben') {
      const { devices: allDevices } = useAppStore.getState();
      const otherSwitchAddresses: string[] = [
        ...(allDevices.switch as SwitchDevice[]).filter(d => d.id !== deviceId).map(d => d.physicalAddress),
        ...actors.map(a => a.physicalAddress)
      ].filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '');
      
      const dimmerAddresses: string[] = (allDevices.dimmer as DimmerDevice[]).map(d => d.physicalAddress).filter((addr): addr is string => typeof addr === 'string' && addr.trim() !== '');
      
      if (otherSwitchAddresses.includes(draft.physicalAddress) || 
            ...
            <div className="flex-between" style={{ marginBottom: 8 }}>
              ...
              <div className="flex" style={{ gap: 8 }}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (confirm(`Weet je zeker dat je ${t('saved')} ${t('switch')} ${actorIdx + 1} wilt verwijderen?`)) {
                      const { removeDevice } = useAppStore.getState();
                      removeDevice('switch', device.id);
                    }
                  }}
                  style={{ fontSize: '0.875rem', padding: '6px 12px' }}
                  title="Verwijderen"
                    ...
                    <div key={output.id} style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 8, backgroundColor: 'var(--color-bg-secondary)', borderRadius: 8 }}>
                      <div className="grid grid-6" style={{ gap: 8 }}>
                        <label className="grid">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updateDraftOutput(device.id, output.id, 'roomAddress', value, true);
                            }}
                        ...
                        <label className="grid">
                          ...
                          <input
                            className="input"
                            placeholder={!output.isReserve ? t('fixturePlaceholder') : undefined}
                        ...
                        {!hasSingleSwitchGroup && (
                          <label className="grid">
                            <span className="small">Groep</span>
                            <select
                              className="input"
                              value={output.switchGroupIndex ?? 0}
                              onChange={(e) => updateDraftOutput(device.id, output.id, 'switchGroupIndex', Number(e.target.value))}
                            >
                              {switchGroups.map((group) => (
                                <option key={`${group.index}-${group.name || ''}`} value={group.index}>
                                  {group.name}
                                </option>
                              ))}
                            </select>
                        ...
                        {isDimmingLinkedToSwitching && dimGroups.length > 0 && (
                          <label className="grid">
                            <span className="small">Groep</span>
                            <select
                              className="input"
              ...
              >
                ...
                <div className="flex-between" style={{ marginBottom: 12 }}>
                  <div className="small" style={{ fontWeight: 'bold' }}>
                    {t('switch')} {existing.length + actorIdx + 1} (nieuw)
                  </div>
                  <button
                    className="button danger"
                    onClick={() => {
                      if (confirm(`Weet je zeker dat je deze nieuwe ${t('switch')} aktor wilt verwijderen?`)) {
                        setActors(prev => {
                            ...
                            {!hasSingleSwitchGroup && (
                              <label className="grid">
                                ...
                                <select
                                  className="input"
                                  value={output.switchGroupIndex ?? 0}
                                  onChange={(e) => updateOutput(actor.id, output.id, 'switchGroupIndex', Number(e.target.value))}
                                >
                                  {switchGroups.map((group) => (
                                    <option key={`${group.index}-${group.name || ''}`} value={group.index}>
                                      {group.name}
                ...
                <div className="flex" style={{ marginTop: 12 }}>
                  ...
                  >
                    {t('save')} {t('switch')} {existing.length + actorIdx + 1}
                  </button>
                </div>
              </div>
            );
